...
if __name__ == '__main__':
	...
	for i, program in enumerate(generator):
		...
		for ex_input, ex_output in data:
			...
			print(repr(program))
			# revised 20170725
			shutil.copy2(setup.SIG_PATH,os.path.join(os.getcwd(),"programs/execute_sig.py"))
			programs.analysis(repr(program),setup.PROGRAM_NAME,setup.SIG_PATH)
			os.remove(os.path.join(os.getcwd(),"programs/execute_sig.py"))
			#
			break
